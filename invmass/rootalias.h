...
const double prl1_min=0.25; // 1.0
const double prl2_min=0.10; // 0.6
...
double tg_dp_L=0.04;
double tg_th_L=0.04;
double tg_ph_L=0.020;//0.03 ; // 0.025
double tg_vz_L=0.09;
double tg_dp_L_min=-0.035;// -0.04;//
double tg_th_L_min=-0.04;
double tg_ph_L_min=-0.035; // -0.03 ;// -0.025
...
const double ps_min=0.12; 
const double sh_min_R=0.15;
...
//TCut l1_cut_R =Form("(R.ps.e/1000.0)>%g",ps_min);
//TCut l2_cut_R =Form("(R.sh.e/1000.0)>%g",sh_min_R);

TCut l1_cut_R =Form("(R.ps.asum_c/1000.0)>%g",ps_min);
TCut l2_cut_R =Form("(R.sh.asum_c/1000.0)>%g",sh_min_R);


const double tg_dp_R=0.05;
const double tg_th_R=0.04;
const double tg_ph_R=0.035;
...
TChain* LoadOnline(Int_t run, const char* path, const char* tree,Int_t debug)
{
    ...
    TString  basename = Form("apex_%d",run);
    TString  rootfile = basename + ".root";
...
TChain* LoadTrim(Int_t run, const char* path, const char* tree,Int_t debug)
{
    TChain* tt = new TChain(tree);

    TString  basename = Form("apex_Trim_%d",run);
    TString  rootfile = basename + ".root";
...
TChain* LoadTrim(Int_t run, const char* tree = "T")
{
    Int_t i=0;

    TChain* tt = 0;
    while (CALPATH[i]) {
  tt = LoadTrim(run,CALPATH[i++],tree,1);
...
Int_t GetTimeStamp(TTree* tt, Int_t decay_days=0)
{
    ...
    if (decay_days==1){
      std::vector<Int_t> filldate={1508774400};// the first tritium cell filled on 2017.10.23. please add newer date in ascending order
        Int_t days = 0;
...
std::vector<string> cut_branches_L ={"L.tr*", "L.cer.asum_c", "L.prl*.e", "Hac*_D1_P0rb", "D*.evtypebits", "EKLx*" };
