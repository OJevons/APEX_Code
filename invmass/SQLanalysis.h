...
string FullTargetName(string tgt){
	std::vector<string> tgt_names  = { "Tritium" ,"Deuterium", "Hydrogen", "Helium-3","Carbon","unknown"};
	std::vector<string> H3  = {"Tritium" ,"T", "T3", "H3" };
	std::vector<string> D2  = {"Deuterium","H2", "D", "D2" };
	std::vector<string> H   = {"Hydrogen","H1", "H" };
	std::vector<string> He3 = {"Helium","Helium-3","He3"};
	std::vector<string> C12 = {"Carbon","C12","C"};
	string TGT="";
...
string shortTgtName(string tgt){
	std::vector<string> tgt_names  = { "H3" ,"D2", "H", "He3","C12","unknown"};
	std::vector<string> H3  = {"Tritium" ,"T", "T3", "H3" };
	std::vector<string> D2  = {"Deuterium","H2", "D", "D2" };
	std::vector<string> H   = {"Hydrogen","H1", "H" };
	std::vector<string> He3 = {"Helium","Helium-3","He3"};
...
//cout << result->GetRowCount() <<" "<< pos <<endl;
  ...
  std::vector<string>  fields = {"density_par_0","density_err_0","density_CV_0","density_CV_1","density_CV_2", "Thickness","Thickness_err"};
  ...
  if(target.type!="gas"){
		...
		//cout << result->GetRowCount() <<" "<< pos <<endl;
		  ...
		  if(target.type=="gas"){
		    ...
		    target.dens_par2 = atof(row->GetField(8));
		    target.dens_err2 = atof(row->GetField(9));
		    std::vector<string>  fields = {"density_par_0","density_err_0","density_CV_0","density_CV_1","density_CV_2"};
		    for(int f=0; f<result->GetFieldCount();f++){
			    if(result->GetFieldName(f) == fields[0]) target.dens_par0=atof(row->GetField(f));
			    if(result->GetFieldName(f) == fields[1]) target.dens_err0=atof(row->GetField(f));
			    if(result->GetFieldName(f) == fields[2]) target.dens_CV0 =atof(row->GetField(f));
		...
		std::vector<int> RLtoint(std::vector<RunList> a, string b=""){
		...
		std::vector<RunList> SQL_Kin_Target_RL(TString kin="", TString tgt=""){
			std::vector<RunList> runlist;
		...
		std::vector<int> SQL_Kin_Target(TString kin="", TString tgt="", string suf=""){
			std::vector<int> runlist;
			if(kin =="" || tgt=="")
			{
				cout << "Please use this function with (kin,tgt),,, example (1,Tritium)" <<"\n\n";
...
//&&tgt!="Carbon"
		...
		std::vector<double> SQLRunCorrection(int runnum, int correction_Level=1,  int debug=1)
		{
		///These will be added as I get more into the DB
		string cols ="livetime_eff, livetime_err, trigger_events, PID_cer_eff,  PID_cer_err, PID_sh_eff, PID_sh_err, PID_ps_eff, PID_ps_err,   Tracking_eff, Tracking_err, Trigger_eff, Trigger_err, PID_NE_eff, PID_NE_err";  //0-3, 4-8,9-10
			...
			std::vector<double> correction_vec={1.0,0.0};
			double correction=1.0;
...
//"livetime_eff, livetime_err, trigger_events, PID_cer_eff,  PID_cer_err, PID_sh_eff, PID_sh_err, PID_ps_eff, PID_ps_err,   Tracking_eff, Tracking_err, Trigger_eff, Trigger_err, PID_NE_eff, PID_NE_err";
		...
		std::vector< std::vector<string>> List_corrections(int runnum)
		{
			std::vector< std::vector<string>> eff_list= {{"0","0"},{"0","0"}};
		...
		std::vector<double> GetPosCorFactor(double xbj, PositronCor PC){
		...
		std::vector<double> GetPosCorFactor(double xbj,int run=0, string tgt="") {
			tgt=FullTargetName(tgt);
			PositronCor PC = GetPosInfo(run,tgt);
			std::vector<double> PC_vec;
			double PCfact = exp(PC.par1+PC.par2*xbj);
...
*/
		...
		//Get the density correction of a target given a current, if you do not provide a current one will be retrieved from SQL if possible.
		std::vector<double> DensityCor( int run, double current=0){
			std::vector<double> dens_vec = {1.0,0.0};
			double err=0;
			double current_err = current_error *current;
			double dens_cor=1;
		...
		//Get the density correction of a target given a current, if you do not provide a current one will be retrieved from SQL if possible.
		std::vector<double> DensityCor( string tgt, double current=22.5){
			std::vector<double> dens_vec = {1.0,0.0};
			double err=0;
			double current_err = current_error *current;
			double dens_cor=1;
		...
		{
		  std::vector <int> list= SQL_Kin_Target(kin,tgt,suf);
